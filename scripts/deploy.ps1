...
# and HeadTracking.ini) next to the RV There Yet exe. Iterates every detected install
...
$steamRoot = $null
try { $steamRoot = Find-GamePath -GameId 'rv-there-yet' } catch { }
...
if ($installs.Count -eq 0) {
    throw "Could not locate RV There Yet (Steam or Xbox). Set RV_THERE_YET_PATH or install via Steam/Xbox."
}
...
foreach ($install in $installs) {
    ...
    foreach ($legacy in "winmm.dll", "dinput8.dll", "xinput1_3.dll", "RVThereYetHeadTracking.asi") {
