...
# End-user installer: enumerates every detected RV There Yet install (Steam +
# Xbox/Game Pass) and deploys to all of them. Called by install.cmd; not
# intended to be run directly by users (the .cmd handles the /y arg parsing +
# pause UX). The dev-tree pixi-run-install path stays on scripts/deploy.ps1 -
# this script is what ships in the release ZIP.
#
...
if (-not $cfg) {
    throw "rv-there-yet not in games.json. cameraunlock-core checkout is incomplete."
}
...
if ($installs.Count -eq 0) {
    Write-Host "ERROR: Could not locate RV There Yet (Steam or Xbox/Game Pass)." -ForegroundColor Red
    Write-Host "Either install via Steam or the Xbox app, set the RV_THERE_YET_PATH env var,"
...
Write-Host ""
Write-Host "=== RV There Yet Head Tracking - Install ===" -ForegroundColor Cyan
...
foreach ($install in $installs) {
    ...
    if (Test-Path $dxgiSrc) {
        if (Test-Path $dxgiDst) {
            $backup = "$dxgiDst.backup"
            if (-not (Test-Path $backup)) {
                Copy-Item $dxgiDst -Destination $backup -Force
                Write-Host "  Backed up original dxgi.dll to dxgi.dll.backup"
            }
        }
