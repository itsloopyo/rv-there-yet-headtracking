...
<#
.SYNOPSIS
  Compare the installed RV There Yet EXE's PE fingerprint against the
  values committed in src/RVThereYetHeadTracking/builds/steam_offsets.cpp.
.DESCRIPTION
...
.PARAMETER ExePath
  Direct path to Ride-Win64-Shipping.exe. If omitted, falls back
...
function Resolve-ExePath {
    ...
    $config = Get-GameConfig -GameId 'rv-there-yet'
    $gameRoot = Find-GamePath -GameId 'rv-there-yet'
    if (-not $gameRoot) {
        throw "Could not locate RV There Yet. Pass the EXE path positionally or set `$env:RV_THERE_YET_PATH."
    }
...
function Read-ExpectedFingerprints {
    ...
    foreach ($file in 'steam_offsets.cpp', 'gdk_offsets.cpp') {
        $cppPath = Join-Path $ProjectDir "src/RVThereYetHeadTracking/builds/$file"
...
# A UE5 shipping EXE's TimeDateStamp is a deterministic-build hash, not a
# timestamp, so newer/older direction can't be inferred from it.
