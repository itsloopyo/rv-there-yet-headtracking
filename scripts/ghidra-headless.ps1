...
# Run a Jython/Python Ghidra script against the RV There Yet shipping EXE, headless.
#
...
# C:\temp\RVThereYet and auto-analyzes. Subsequent runs reuse the project.

param(
    [Parameter(Mandatory = $true)][string]$Script,
    # Optional override for the binary to analyse. Defaults to the Steam
    # RV There Yet EXE. Pass the rebased WinGDK dump from scripts/dump-running-exe.ps1
    # to discover RVAs for the Xbox/GDK profile (which lives behind a Process
    # Trust ACE that blocks static read of the real EXE).
    [string]$ProgramExe = "C:\Program Files (x86)\Steam\steamapps\common\Ride\Ride\Binaries\Win64\Ride-Win64-Shipping.exe",
