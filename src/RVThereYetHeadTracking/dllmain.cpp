...
BOOL APIENTRY DllMain(HMODULE hModule, DWORD reason, LPVOID lpReserved)
{
    if (reason == DLL_PROCESS_ATTACH) {
        ...
        RVThereYetHeadTracking::Initialize(hModule);
        ...
        // they held in a "locked" state - so our full Shutdown (which joins
        // worker threads and acquires log/hook locks) would deadlock or
        // worse. The OS reclaims handles, sockets, and memory on exit, so
        // the only thing worth doing on that path is a final log line.
        if (lpReserved == nullptr) {
            RVThereYetHeadTracking::Shutdown();
        } else {
            RVThereYetHeadTracking::EmergencyShutdown();
        }
    }
    return TRUE;
}
